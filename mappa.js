...
function color(st,country){if(country&&st)return "#050505";return st==="cucci"?"#b3132b":st==="cicci"?"#6d1b7b":st==="both"?"#050505":"#555158"}
...
function renderWorld(){
  ...
  const rows=[...by.entries()].sort(function(a,b){return (a[1][0].countryName||a[0]).localeCompare(b[1][0].countryName||b[0],"it")}).map(function(x){const c=x[0],l=x[1],n=l[0].countryName||c;return '<div class="visit-card both" onclick="openCountry(\''+esc(c)+'\',\''+esc(n)+'\')"><div class="visit-title">'+esc(n)+'</div><div class="visit-meta">'+l.length+' '+(l.length===1?"permanenza":"permanenze")+' · '+esc(who([].concat.apply([],l.map(function(v){return v.visitors||[]}))))+'</div></div>'}).join("");
  document.getElementById("sidePanel").innerHTML='<div class="eyebrow">Mappa condivisa</div><h2>Mondo</h2><p class="muted">I paesi visitati sono neri. Aprine uno per vedere il dettaglio di regioni e città.</p><div class="summary"><div class="stat"><strong>'+by.size+'</strong><span>paesi</span></div><div class="stat"><strong>'+cityCount+'</strong><span>città</span></div><div class="stat"><strong>'+days+'</strong><span>giorni segnati</span></div></div><div class="section-title">Paesi visitati</div>'+(rows||'<div class="empty">Ancora nessun paese segnato.</div>');
}
